    ...
    if sp.is_mysql():
        sql = """
            SELECT
                ...
                (SELECT IFNULL(SUM(STOCK), 0) FROM PRODUCTO WHERE ESTADO = 'Activo') AS STOCK_TOTAL,
                ...
                (SELECT COUNT(*) FROM VENTA WHERE ESTADO = 'Pagado'
                    AND FECHACREACION = fn_fecha_ddmmyyyy()) AS VENTAS_HOY,
                (SELECT IFNULL(SUM(TOTAL), 0) FROM VENTA WHERE ESTADO = 'Pagado'
                    AND FECHACREACION = fn_fecha_ddmmyyyy()) AS MONTO_HOY,
